...
### Scheduler
        ...
        self.tw_Schedule = QTableWidget(0,10,self.page_2)
        self.tw_Schedule.setHorizontalHeaderLabels(['파일',"방송구간","날짜","시간","반복","활성화","수정","복사" ])
        self.tw_Schedule.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.tw_Schedule.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.tw_Schedule.horizontalHeader().setStretchLastSection(True)
        self.tw_Schedule.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        #self.tw_Schedule.setMaximumSize(QSize(600, 16777215))
        #self.tw_Schedule.setStyleSheet("QWidget{color:#ffffff}; QTableWidget{gridline-color: #ffffff};")
        self.tw_Schedule.setFrameShape(QFrame.NoFrame)
        #self.tw_Schedule.setFrameShadow(QFrame.Plain)
        self.tw_Schedule.setStyleSheet("""
                                        QWidget{background-color:#f8f9f9}
                                        QHeaderView::section{padding:5px;border:0px}
                                        QTableView{selection-color:white;selection-background-color:#566573}
                                        QTableView QTableCornerButton::section {background:#f8f9f9}
        ...
        self.gl_Schedule.addWidget(self.tw_Schedule, 0, 0)
        ...
        self.hbox_Schedule_Add_Remove = QHBoxLayout(self.page_2)
        self.vbox_Schedule.addLayout(self.hbox_Schedule_Add_Remove)
        self.btn_schedule_Add = QPushButton('ADD', self.page_2)
        self.btn_schedule_Add.setMinimumSize(0,30)
        self.btn_schedule_Add.setStyleSheet("""
                                            QPushButton{border:none;border-radius:10px;background-color:#307C8C}
                                            QPushButton:hover{background-color:#22BCDD}
                                            QPushButton:pressed{color:white;background-color:#485152}
                                            """)

        self.btn_schedule_Remove = QPushButton('REMOVE', self.page_2)
        self.btn_schedule_Remove.setMinimumSize(0,30)
        self.btn_schedule_Remove.setStyleSheet("""
                                            QPushButton{border:none;border-radius:10px;background-color:#BD342D}
                                            QPushButton:hover{background-color:#FF0C00}
                                            QPushButton:pressed{color:white;background-color:#745453}
                                            """)


        self.hbox_Schedule_Add_Remove.addWidget(self.btn_schedule_Add)
        self.hbox_Schedule_Add_Remove.addWidget(self.btn_schedule_Remove)
...
### 3rd Line
    ###Player Control
        ...
        #self.playControlButtons.addButton(self.btn_Play,1)
        ...
        #self.playControlButtons.addButton(self.btn_RW,2)
        ...
        #self.playControlButtons.addButton(self.btn_Stop,0)
        ...
        #self.playControlButtons.addButton(self.btn_FF,3)
        ...
        #self.playControlButtons.addButton(self.btn_Loop,4)
...
class Dialog_Zone_Sel(object):
    def setupUi(self,Dialog):
        ...
        self.lbl_popup_title = QLabel("스케쥴러 설정",Dialog)
        ...
        self.hbox_popup = QHBoxLayout()
        self.vbox_popup.addLayout(self.hbox_popup)

        self.gl1_popup = QGridLayout()
        self.gl1_popup.setContentsMargins(10,0,10,0)
        self.hbox_popup.addLayout(self.gl1_popup)

        self.lbl_zone_sel_popup = QLabel("지역선택",Dialog)
        self.lbl_zone_sel_popup.setMinimumSize(100,30)
        self.lbl_zone_sel_popup.setFont(font_Label)
        self.lbl_zone_sel_popup.setStyleSheet("background-color:white;padding-left:30px")
        self.gl1_popup.addWidget(self.lbl_zone_sel_popup,0,0,1,6)

        for i in range(66):
            self.btn_zone_sel[i] = QPushButton("{}".format(i+1), Dialog)
            self.btn_zone_sel[i].setMinimumSize(QSize(80, 40))
            ...
            self.gl1_popup.addWidget(self.btn_zone_sel[i],int(i/6)+1,int(i%6),1,1)

        self.gl2_popup = QGridLayout()
        self.gl2_popup.setContentsMargins(10,0,10,0)
        self.hbox_popup.addLayout(self.gl2_popup)

        self.lbl_file_load = QLabel("파일선택", Dialog)
        #self.lbl_file_load.setMaximumHeight(30)
        self.lbl_file_load.setFont(font_Label)
        self.lbl_file_load.setStyleSheet("background-color:white;padding-left:30px")
        self.gl2_popup.addWidget(self.lbl_file_load, 0,0,1,2)

        self.l_edit_file_load = QLineEdit(Dialog)
        self.l_edit_file_load.setStyleSheet("border:none")
        self.gl2_popup.addWidget(self.l_edit_file_load,1,0,1,1)

        self.btn_file_load = QPushButton(Dialog)
        self.btn_file_load.setIcon(icon2)
        self.btn_file_load.setFlat(True)
        self.btn_file_load.setMaximumSize(QSize(30, 100))
        self.btn_file_load.setFocusPolicy(Qt.NoFocus)
        self.gl2_popup.addWidget(self.btn_file_load,1,1,1,1)

        self.lbl_schedule_set = QLabel("반복선택", Dialog)
        #self.lbl_schedule_set.setMaximumHeight(30)
        self.lbl_schedule_set.setFont(font_Label)
        self.lbl_schedule_set.setStyleSheet("background-color:white;padding-left:30px")
        self.gl2_popup.addWidget(self.lbl_schedule_set, 2,0,1,2)

        grp_schdeule = QGroupBox()
        grp_schdeule.setMaximumHeight(50)
        self.gl2_popup.addWidget(grp_schdeule, 3,0,1,2)
        hbox = QHBoxLayout()
        grp_schdeule.setLayout(hbox)

        self.lbl_schedule_set_ones = QRadioButton("한번",Dialog)
        self.lbl_schedule_set_ones.setChecked(True)
        self.lbl_schedule_set_times = QRadioButton("매시간",Dialog)
        self.lbl_schedule_set_days = QRadioButton("매일", Dialog)
        self.lbl_schedule_set_wdays = QRadioButton("매주", Dialog)
        
        hbox.addWidget(self.lbl_schedule_set_ones )
        hbox.addWidget(self.lbl_schedule_set_times)
        hbox.addWidget(self.lbl_schedule_set_days )
        hbox.addWidget(self.lbl_schedule_set_wdays)

        hbox1 = QHBoxLayout()
        self.gl2_popup.addLayout(hbox1, 4,0,1,2)

        self.lbl_popup_sch_date = QLabel("날짜",Dialog)
        self.lbl_popup_sch_date.setAlignment(Qt.AlignCenter|Qt.AlignVCenter)
        hbox1.addWidget(self.lbl_popup_sch_date)
        self.qde_popup_sch_date = QDateEdit(Dialog)
        self.qde_popup_sch_date.setMinimumSize(100,30)
        self.qde_popup_sch_date.setStyleSheet("QDateEdit{border:none;padding:5px}")
        hbox1.addWidget(self.qde_popup_sch_date)
        self.lbl_popup_sch_time = QLabel("시간",Dialog)
        self.lbl_popup_sch_time.setAlignment(Qt.AlignCenter|Qt.AlignVCenter)
        hbox1.addWidget(self.lbl_popup_sch_time)
        self.qte_popup_sch_time = QTimeEdit(Dialog)
        self.qte_popup_sch_time.setMinimumSize(100,30)
        self.qte_popup_sch_time.setStyleSheet("QTimeEdit{border:none;padding:5px}")
        hbox1.addWidget(self.qte_popup_sch_time)

        self.lbl_sch_time = QLabel("시간 제한 설정",Dialog)
        self.lbl_sch_time.setMinimumHeight(30)
        self.lbl_sch_time.setFont(font_Label)
        self.lbl_sch_time.setStyleSheet("background-color:white;padding-left:30px")
        self.gl2_popup.addWidget(self.lbl_sch_time, 5,0,1,1)
        self.btn_sch_time = QPushButton("활성화",Dialog)
        self.btn_sch_time.setMinimumSize(QSize(80,30))
        self.btn_sch_time.setCheckable(True)
        self.btn_sch_time.setStyleSheet(self.btn_style_nomal)
        self.gl2_popup.addWidget(self.btn_sch_time, 5,1,1,1)

        hbox2 = QHBoxLayout()
        self.gl2_popup.addLayout(hbox2, 6,0,1,2)
        lbl_stx_time = QLabel("시작시간")
        lbl_stx_time.setAlignment(Qt.AlignCenter|Qt.AlignVCenter)
        hbox2.addWidget(lbl_stx_time)
        self.qte_stx_time = QTimeEdit(Dialog)
        self.qte_stx_time.setMinimumSize(100,30)
        self.qte_stx_time.setStyleSheet("QTimeEdit{border:none;padding:5px}")
        hbox2.addWidget(self.qte_stx_time)
        lbl_etx_time = QLabel("종료시간")
        lbl_etx_time.setAlignment(Qt.AlignCenter|Qt.AlignVCenter)
        hbox2.addWidget(lbl_etx_time)
        self.qte_etx_time = QTimeEdit(Dialog)
        self.qte_etx_time.setMinimumSize(100,30)
        self.qte_etx_time.setStyleSheet("QTimeEdit{border:none;padding:5px}")
        hbox2.addWidget(self.qte_etx_time)

        self.lbl_sch_wday = QLabel("요일 설정",Dialog)
        #self.lbl_sch_wday.setMaximumHeight(30)
        self.lbl_sch_wday.setFont(font_Label)
        self.lbl_sch_wday.setStyleSheet("background-color:white;padding-left:30px")
        self.gl2_popup.addWidget(self.lbl_sch_wday, 7,0,1,2)

        hbox3 = QHBoxLayout()
        self.gl2_popup.addLayout(hbox3, 8,0,1,2)

        self.btn_wdays_0 = QPushButton("월", Dialog)
        self.btn_wdays_1 = QPushButton("수", Dialog)
        self.btn_wdays_2 = QPushButton("목", Dialog)
        self.btn_wdays_3 = QPushButton("화", Dialog)
        self.btn_wdays_4 = QPushButton("금", Dialog)
        self.btn_wdays_5 = QPushButton("토", Dialog)
        self.btn_wdays_6 = QPushButton("일", Dialog)

        wday_btns = [self.btn_wdays_0,self.btn_wdays_1,self.btn_wdays_2,self.btn_wdays_3,self.btn_wdays_4,self.btn_wdays_5,self.btn_wdays_6]
        for i in range(len(wday_btns)):
            wday_btns[i].setMaximumSize(QSize(50, 50))
            wday_btns[i].setCheckable(True)
            wday_btns[i].setStyleSheet(self.btn_style_nomal)
            hbox3.addWidget(wday_btns[i])

        self.lbl_sch_act = QLabel("스케쥴 활성화",Dialog)
        self.lbl_sch_act.setMinimumHeight(40)
        self.lbl_sch_act.setFont(font_Label)
        self.lbl_sch_act.setStyleSheet("background-color:white;padding-left:30px")
        self.gl2_popup.addWidget(self.lbl_sch_act, 9,0,1,1)
        self.btn_sch_act = QPushButton("활성화",Dialog)
        self.btn_sch_act.setMinimumSize(QSize(80,30))
        self.btn_sch_act.setCheckable(True)
        self.btn_sch_act.setStyleSheet(self.btn_style_nomal)
        self.gl2_popup.addWidget(self.btn_sch_act, 9,1,1,1)

        self.lbl_blank_1 = QLabel(Dialog)
        self.lbl_blank_1.setMinimumHeight(70)
        self.gl2_popup.addWidget(self.lbl_blank_1, 10,0,1,2)
        self.lbl_blank_2 = QLabel(Dialog)
        self.lbl_blank_2.setMinimumHeight(70)
        self.gl2_popup.addWidget(self.lbl_blank_2, 11,0,1,2)
